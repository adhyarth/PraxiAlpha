"""
SMH Overextension Analysis

Find all instances when SMH weekly high was >= 104.8% above the 200-week SMA
(same extension as the week ending April 10, 2026: high 441.54, 200W SMA 215.59).
...
async def main():
    ...
    current_high = 441.54
    current_200w_sma = 215.59
    current_extension_pct = (current_high - current_200w_sma) / current_200w_sma * 100
    print(f"Current SMH extension: {current_extension_pct:.2f}% above 200W SMA")
    print(f"  High: ${current_high:.2f}, 200W SMA: ${current_200w_sma:.2f}")
    print()

    async with async_session_factory() as session:
        # Get SMH stock_id
        result = await session.execute(text("SELECT id FROM stocks WHERE ticker = 'SMH' LIMIT 1"))
        row = result.fetchone()
        if not row:
            print("ERROR: SMH not found in database")
            return
        stock_id = row.id

        # Fetch weekly candles for SMH (from the weekly_ohlcv view)
    ...
    if not rows:
        print("ERROR: No weekly data found for SMH")
        return
    ...
    print(f"Loaded {len(df)} weekly candles for SMH")
    ...
    # Filter: extension >= current extension (104.8%)
    threshold = current_extension_pct
    signals = df[df["extension_pct"] >= threshold].copy()

    print(f"Found {len(signals)} weeks where SMH high was >= {threshold:.2f}% above 200W SMA")
    print()
    ...
    # Print to console
    print("=" * 120)
    print(f"SMH Overextension Instances (High >= {threshold:.2f}% above 200W SMA)")
    print("=" * 120)
    ...
    output_path = output_dir / "smh_overextension_analysis.csv"
    results_df.to_csv(output_path, index=False)
    print(f"Results saved to: {output_path}")

    # Also save summary
    summary_path = output_dir / "smh_overextension_summary.csv"
