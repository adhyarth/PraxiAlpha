...
### Changed
- **Migrated validation from tvdatafeed to yfinance** — replaced `fetch_tv_candles()` + `get_tv_client()` with `fetch_yf_candles()`. Removed tvdatafeed dependency (unmaintained 4yr-old websocket scraper with ~50% TCP failure rate) in favor of `yfinance` (stable PyPI package, REST API, 12K+ stars, actively maintained). All existing validation logic (comparison, metadata enrichment, Streamlit UI, tests) preserved — only the data-fetch layer swapped.
...
- **Deleted obsolete tvdatafeed scripts** — removed `scripts/validate_tradingview.py`, `scripts/debug_aapl_volume.py`, and `scripts/debug_volume_multi.py` (debugging tools specific to tvdatafeed websocket issues).
- **Streamlit validation UI updated** — all labels changed from "TradingView" to "Yahoo Finance", removed TV credential verification step, reduced rate-limit sleep from 2s to 0.5s (REST API vs websocket).
- **Data Validation service** (`backend/services/tv_validation_service.py`) — backend service for comparing OHLCV data (daily, weekly, monthly, quarterly) between PraxiAlpha's database and Yahoo Finance. Includes bar-by-bar comparison with configurable tolerances (1% price, 10% volume), quarterly aggregation from YF monthly data, failure persistence (JSON), and summary computation.
- **43 validation tests** (`backend/tests/test_tv_validation.py`, 494 total) — all tests pass with no changes to test logic (tests exercise comparison/aggregation/persistence, not the fetch layer).
